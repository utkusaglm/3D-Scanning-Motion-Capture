...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
        ...
        Eigen::Vector3d diff = _x - m_center;
        return diff.norm() - m_radius;
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
        ...
        Eigen::Vector3d x_c = (_x - m_center);
        double eq = (_x - m_center).transpose() * (_x - m_center) + m_radius * m_radius - m_a * m_a;
        double xy = pow((_x.x() - m_center.x()),2) + pow((_x.y() - m_center.y()),2);
        return pow(eq,2) - 4 * pow(m_radius,2) * xy;
	}
